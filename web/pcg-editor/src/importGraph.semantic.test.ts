  ...
  it('preserves validated node and Subgraph semantics', () => {
    const parsed = parseGraphJson(JSON.stringify({
      version: '2.0', nodes: [{ id: 'table', type: 'CreateBoxMesh', position: { x: 0, y: 0 }, data: {
        __semantic: { componentId: 'interior_table', bounds: { center: [-2, 1, -2], size: [2, 1, 1] }, camera: { occluder: true } },
      } }], edges: [], subgraphs: [{
    ...
    if (!parsed.ok) return;
    expect(parsed.nodes[0].data.__semantic).toMatchObject({ componentId: 'interior_table' });
