...
{
    /// <summary>
    /// Studio rock turntable + explode-parts presentation for the Glock-18 PCG assembly.
    /// Splits cooked submeshes into child parts and animates separation on demand.
    /// </summary>
    [ExecuteInEditMode]
    public sealed class GhostProtocolGlockPresenter : MonoBehaviour
    {
        [SerializeField] private PcgGraphComponent graph;
        [SerializeField] private bool enableStudioRock = true;
        [SerializeField] private float studioRockDegreesPerSecond = 8f;
        ...
        /// <summary>Inspector / UI button: toggle exploded view.</summary>
        ...
        private void TrySplitSubmeshes()
        {
            ...
            {
                var partGo = new GameObject($"Part_{i}");
                ...
                var sub = new Mesh { name = $"GlockPart_{i}" };
        ...
        private static Vector3 ResolveExplodeOffset(int index, Material[] materials)
        {
            if (index < materials.Length && materials[index] != null)
            {
                ...
                if (name.Contains("inner_core"))
                    return Glock18ProfileAdapter.ExplodeOffsets["slide_shell"];
                if (name.Contains("ruby_shell"))
                    return Glock18ProfileAdapter.ExplodeOffsets["shell_outer"];
                if (name.Contains("dark_metal"))
                    return Glock18ProfileAdapter.ExplodeOffsets["internals"];
                if (name.Contains("trigger_polymer"))
                    return Glock18ProfileAdapter.ExplodeOffsets["trigger_group"];
                if (name.Contains("ruby_glow"))
                    return Glock18ProfileAdapter.ExplodeOffsets["frame_receiver"];
            }

            var offsets = Glock18ProfileAdapter.ExplodeOffsets.Values.ToList();
...
#if UNITY_EDITOR
namespace DJTechRuntime.PCG.Examples.GhostProtocolGlock.Editor
{
    using UnityEditor;

    [CustomEditor(typeof(GhostProtocolGlockPresenter))]
    public sealed class GhostProtocolGlockPresenterEditor : Editor
    {
        public override void OnInspectorGUI()
        {
            DrawDefaultInspector();
            var p = (GhostProtocolGlockPresenter)target;
