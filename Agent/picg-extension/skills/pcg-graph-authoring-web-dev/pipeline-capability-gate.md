# PCG pipeline validation gate (web)
...
It validates the generator itself. It does not replace geometry validation, UV/projection work, materials, textures, export, rendering, or final acceptance; those use the same shared rules as the standard skill.
...
Load the AssetSpec, the shared `pcg-graph-authoring.md`, the current `schema/node-manifest.json`, applicable PCG rules, and the generated graph. Golden Graphs under `VAULT_ROOT/PCG AI Rule/Golden Graphs/` may support an allowed composition; repo examples and web demos may not.
...
Run every relevant row against the authored graph and a clean web review page.
...
| Durability | Asset paths, exported asset/graph references, clean-page recook and reload | No session-instance-only reference, broken path, duplicate output, or stale artifact survives regeneration |

Use the clean review page and capture procedure from `AUTHORING_SKILL_DIR/web-review.md`. The gate may use a white model to inspect structure and variation, but it must not score artistic material fidelity; full material/texture scoring remains in shared final acceptance.
...
evidence: graph=<path> | reviewPage=<path> | reports=<paths>
```
