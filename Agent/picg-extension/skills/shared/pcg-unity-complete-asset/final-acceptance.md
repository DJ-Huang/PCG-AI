...
Score the fully textured Unity prefab in the final clean-scene render. Do not score a white model, a graph canvas, an untextured mesh, placeholder materials, or a default Unity material as the final asset.

## Shared scorecard

Use these weights in both skills:
...
| Technical asset construction | 0.15 | valid graph/output, hierarchy, pivot, parameters, and reproducible prefab |
| Unity integration | 0.10 | imports, shaders, bindings, durable paths, recook/reload behavior |
...
- missing, pink, default, or placeholder material on a visible final renderer;
- required texture absent, wrongly imported/bound, visibly invalid mapping, or unresolved shader error;
- prefab missing/damaged, unstable on reload, or holding broken references;
- final render not from the fully textured prefab in a clean review scene;
...
Unity integration: <0..1> × 0.10
Presentation: <0..1> × 0.10
Score: <0..1>
Evidence: prefab=<path> | render=<path> | comparison=<path>
