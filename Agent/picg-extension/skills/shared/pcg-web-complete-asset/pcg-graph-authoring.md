...
4. Glob/read `VAULT_ROOT/PCG AI Rule/Golden Graphs/<class>/` from disk. That directory is `.ragignore`; do not RAG-search it. Do not mine repo `examples/**/*.pcg` or demos for new authoring. Use `examples.md` only for short pin-wiring templates. Exception: open a graph the user explicitly asked to edit.
