---
name: pcg-graph-authoring-unity-dev
description: >-
  Create the same complete production-ready Unity assets as
  pcg-graph-authoring-unity—PCG graph, cooked and validated white model,
  UV/projection, materials, textures, Unity bindings, prefab, final render, and
  full-asset acceptance—while additionally validating the PCG generator after
  graph authoring for structure, parameters, seeds, boundary inputs,
  regeneration, performance, and AssetSpec-consistent output. It also runs a
  deterministic root/Subgraph layout pass before graph validation. Use for
  `/pcg-graph-authoring-unity-dev`, PCG pipeline validation, capability-gap
...
# PCG Unity complete-asset production — development validation

Produce the exact same finished asset and use the exact same acceptance standard as `pcg-graph-authoring-unity`. This is a strict superset: it adds one PCG-specific validation gate; it does not replace the material, texture, prefab, render, or final scoring workflow.
...
AUTHORING_SKILL_DIR = ../pcg-graph-authoring-unity
DEV_SKILL_DIR = .
```
...
3. For reference-image work, read `AUTHORING_SKILL_DIR/llm-orchestration.md`, `scripts.md`, and `unity-review.md` before the first graph write or visual cook.
4. Read the same shared stage references as the standard skill immediately before geometry, material, texture, Unity, and final-acceptance stages. Recover through `SHARED_DIR/error-codes.md`.
...
When the same graph is open in the Web editor, use PCG MCP after static validation for native live validation and fixed-seed cook receipts. Repeat cook/tuning across the gate's required seeds or boundary values only when the state can be changed safely through existing node data, verify every queued patch actually applies, and restore the final intended state. If the live graph differs or structural edits are required, use the deterministic file/HTTP path; never weaken the gate to fit the current MCP surface. Tuanjie/Unity clean-scene evidence remains mandatory for Unity acceptance.
...
Include the dev pipeline report beside—not instead of—the shared geometry, UV, material, prefab, render, and final-score report. The PCG gate adds technical evidence only; it must not alter the standard artistic/technical score weights or acceptance threshold.
