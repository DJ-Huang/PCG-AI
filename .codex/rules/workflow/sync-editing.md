...
- [ ] 改门禁（`gates/rule-router-gate.md`、`codely-pre-code-gate.md`、`file-write-integrity.md`、`codely-shared.md`）→ 对照现有 gate 的 frontmatter（`description`、`alwaysApply`）与占位符 `.codex/rules`、`~/.codex/skills`（由 `scripts/rule-format.mjs` 在同步时替换）。
...
- [ ] 引用规则用相对路径或 `.codex/rules`，**不复制**规则正文到 Skill。
