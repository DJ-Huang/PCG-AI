...
1. For visual work, select and ping the workspace Unity instance, then follow `AUTHORING_SKILL_DIR/unity-review.md` end-to-end. Always use a new `Assets/PICG-Workspace/Scenes/PcgReview_<slug>.scene`; do not judge a graph in a demo/cluttered scene.
...
previewScene=Assets/PICG-Workspace/Scenes/PcgReview_<slug>.scene | loop=on
```
