# PCG Unity complete-asset workflow

This is the single production workflow for both `pcg-graph-authoring-unity` and `pcg-graph-authoring-unity-dev`. Do not treat a `.pcg`, a cooked procedural mesh, or a white model as the finished result.

`AUTHORING_SKILL_DIR` means the sibling `pcg-graph-authoring-unity` skill directory. `SHARED_DIR` means this directory. Resolve these paths before reading any reference; do not recreate a local copy when a shared reference is unavailable.
...
→ material plan
→ texture production and Unity import
...
3. Read the stage reference immediately before that stage:
   ...
   - [unity-integration.md](unity-integration.md) before creating material assets, bindings, a prefab, or a final render;
...
For a reference-image job, also read `AUTHORING_SKILL_DIR/llm-orchestration.md`, `AUTHORING_SKILL_DIR/scripts.md`, and `AUTHORING_SKILL_DIR/unity-review.md` before the first graph write or visual review.
...
| `MATERIALS_BOUND` | Material plan, imported texture assets, Unity material assets, and bindings | no missing, pink, default, or placeholder material remains |
| `PREFAB_READY` | Re-openable prefab with stable hierarchy and asset references | prefab regenerates without stale or duplicate output |
| `FINAL_ACCEPTED` | Fully textured render sheet and final acceptance report | shared score and every hard-failure check pass |

## Autonomous behavior

Apply practical defaults for graph parameters, save directory, material values, texture size, and review scene. Continue through all stages without asking for confirmation. Record assumptions and receipts in the plan/final reply.

Ask only when an input is genuinely unusable, the selected Unity instance is absent or ambiguous, a required source asset is unavailable, or the user explicitly requests interactive control. A real pipeline capability gap in the development variant is also a blocker; see its gate reference.
...
Prefab: <path> | hierarchy=PASS | regeneration=PASS
