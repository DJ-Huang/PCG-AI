...
**禁止**用 `vault_search` 代替规则检索；**禁止** `Read` `.trae/rules/core/`、`agents/`、`reviews/`（本地不应存在这些目录；项目级规则文件如 `pcg-ai.md` 放 `rules/` 根目录，不放子目录）。

**每次**修改产品路径（见 `gates/codely-pre-code-gate`）**之前**：

1. `user-vault-rag.rule_search(query=任务技术关键词 + 意图, domain=unity|comfyui, top_k=10)`（**通用**域名传 `domain=null`）
...
`user-vault-rag.vault_search(query=任务技术关键词 + 任务类型, top_k=10)` — Pitfall / Tip / Concept / BugFix。规则正文必须用 §2 `rule_search`（仅 `Rules/`）。

与 §2 **同一次写码前**各执行一次（先 rule_search，再 vault_search）。
...
`Router（dev）：<域名> | rules: <rule_search 命中 rule_id 列表> | vault: <vault_search 命中摘要或 no hit>`
